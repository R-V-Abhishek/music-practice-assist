...
\definecolor{tableheadbg}{RGB}{30,80,150}
\definecolor{tablerowA}{RGB}{240,245,255}
...
\section{Background}
In Carnatic music, every performance is based on a performer-specific tonic Sa, with all other swaras defined relative to it. Each raga follows strict rules, and even small pitch deviations or incorrect notes can affect its identity.

Traditionally, students learned under close guidance in a gurukula-style system. However, in today’s world, such continuous mentorship is not always feasible, and students often practice alone. This makes it difficult to detect mistakes like pitch inaccuracies and violations of raga grammar, leading to inconsistent learning.

Existing raga recognition systems mainly focus on classification after performance and do not provide real-time feedback or handle microtonal precision effectively. They also treat tonic detection as a secondary step.

This project addresses these gaps by building a real-time assistant that prioritizes tonic detection, tracks pitch accurately, and validates raga grammar. It acts as an automated practice companion, helping students improve pitch fidelity and maintain raga correctness during practice.
...
\arrayrulecolor{tableheadbg}
\toprule
\rowcolor{tableheadbg}
\textcolor{white}{\textbf{Reference}} &
\textcolor{white}{\textbf{Focus}} &
\textcolor{white}{\textbf{Relevance to this work}} \\
\midrule
\arrayrulecolor{black!15}
\rowcolor{tablerowA}
\cite{hebbar2023comparison} Hebbar \& Jagtap & CNN on Mel-spectrograms (98.1\% on CMD-40) & Confirms spectral features are discriminative; motivates our spectral tonic scorer \\
\rowcolor{tablerowB}
\cite{krishnaswamy2018application} Krishnaswamy & Carnatic F0 via STFT & Establishes tonic-relative pitch extraction as a prerequisite \\
\rowcolor{tablerowA}
\cite{peri2023applying} Peri & NLP-style hierarchical attention on raga sequences & Motivates grammar-aware sequence validation \\
\rowcolor{tablerowB}
\cite{natesan2020carnatic} Natesan \& Beigi & TDNN+LSTM+attention (95.31\% on 172 ragas) & Supports temporal phrase modeling in FSM \\
\rowcolor{tablerowA}
\cite{arxiv2022carnatic} Stacked ensemble & Multi-modal feature fusion & Motivates Carnatic HPS cross-validation ensemble \\
\rowcolor{tablerowB}
\cite{ic2021comparative} ICIDS 2021 & Classifier benchmarks for raga & Confirms feasibility of sub-50\,ms processing \\
\rowcolor{tablerowA}
\cite{ieee2022deep} IEEE Trans.\ Multimedia & DCNN spectral feature learning & Demonstrates deep feature hierarchies for raga \\
\rowcolor{tablerowB}
\cite{ismir2023explainable} ISMIR 2023 & XAI for raga identification & Motivates transparent, teacher-like feedback \\
\rowcolor{tablerowA}
\cite{jasi2021investigation} JASI 2021 & Acoustic pitch distribution study & Informs vivaadi swara definitions \\
\rowcolor{tablerowB}
\cite{ismir2020melody} ISMIR 2020 & Real-time polyphonic melody extraction & Confirms sub-50\,ms latency is achievable \\
\rowcolor{tablerowA}
\cite{ijca2015phrase} IJCA 2015 & Pitch distribution phrase analysis & Precursor to prayoga phrase buffer \\
\rowcolor{tablerowB}
\cite{ncc2022raga} NCC 2022 & t-SNE raga space visualization & Grounds 22-shruti quantization mathematically \\
\rowcolor{tablerowA}
\cite{ieee2021self} IEEE Sig.\ Proc.\ Lett. & Self-extracting adaptive feature maps & Motivates median-spectrum robust scoring \\
\rowcolor{tablerowB}
\cite{icassp2019singing} ICASSP 2019 & Vocal separation + pitch (30--40\,ms) & Confirms real-time pitch extraction latency \\
\rowcolor{tablerowA}
\cite{prl2021vector} Pattern Recog.\ Lett. & Vector space 22-shruti model (88$\to$94\%) & Core foundation for cent-deviation swara mapping \\
\arrayrulecolor{tableheadbg}
\bottomrule
\end{tabular}
\end{table}
...
\arrayrulecolor{black!15}
\rowcolor{tablerowA}
Literature review        & Oct--Nov 2025    & Survey and gap analysis complete \\
\rowcolor{tablerowB}
Algorithm design         & Nov--Dec 2025    & Tonic detection and swara mapping designed \\
\rowcolor{tablerowA}
Core implementation      & Jan 2026    & HPS ensemble and swara quantizer built \\
\rowcolor{tablerowB}
FSM and grammar          & Jan--Feb 2026    & Multi-phase validator complete \\
\rowcolor{tablerowA}
UI integration           & Mar 2026    & WebSocket dashboard live \\
\rowcolor{tablerowB}
Testing and tuning       & Mar 2026    & All test suites pass \\
\rowcolor{tablerowA}
Thesis writing           & Apr 2026    & Draft chapters submitted to supervisor \\
\rowcolor{tablerowB}
Review and corrections   & Apr 2026           & Supervisor feedback incorporated \\
\rowcolor{tablerowA}
Final submission         & May 2026           & Hard copy and USB submitted \\
\arrayrulecolor{tableheadbg}
\bottomrule
\end{tabular}
\end{table}
...
\begin{tikzpicture}[
  ...
  layer/.style     = {rectangle, rounded corners=4pt, draw=#1!70!black,
                      fill=#1!18, minimum width=3.2cm, minimum height=0.85cm,
                      align=center, font=\small\bfseries, drop shadow},
  module/.style    = {rectangle, rounded corners=3pt, draw=#1!80!black,
                      fill=#1!25, minimum width=3.2cm, minimum height=0.85cm,
...
%% ── ROW 1 : INPUT ──────────────────────────────────────────────────────────
\node[layer=gray] (mic)
      {Microphone \\ \small (Browser / Desktop)};

%% ── ROW 2 : AUDIO LAYER ────────────────────────────────────────────────────
\node[module=teal, below=of mic] (lap)
      {\texttt{live\_audio\_processor.py} \\ \scriptsize PCM ingest · resampling · slope gate};

%% ── ROW 3 : PIPELINE LAYER ─────────────────────────────────────────────────
\node[module=blue, below=of lap] (pip)
      {\texttt{pitch\_pipeline.py} \\ \scriptsize frame coordinator · bootstrap buffer};

%% ── ROW 4 : THREE PARALLEL MODULES (wider spread to avoid collision) ───────
\node[module=orange, below left=0.55cm and 4.2cm of pip]  (tsa)
...
\node[module=green!60!black, below right=0.55cm and 4.2cm of pip] (gv)
      {\texttt{grammar\_validator.py} \\ \scriptsize 5-phase FSM \\ \scriptsize varja · vakra · prayoga};

%% ── RAGA DATABASE : placed to the right of gv with generous gap ────────────
\node[module=yellow!70!black, right=2.0cm of gv] (rdb)
...
%% ── ROW 6 : WEB LAYER ──────────────────────────────────────────────────────
\node[module=teal!60!black, below=of fg] (web)
      {\texttt{web/app.py} \\ \scriptsize FastAPI · WebSocket server};

%% ── ROW 7 : OUTPUT ─────────────────────────────────────────────────────────
\node[layer=gray, below=of web] (ui)
...
%% pip -> three parallel modules (fan-out)
...
%% swq + gv -> feedback generator (fan-in)
...
%% raga DB -> grammar validator (straight horizontal from left side of rdb)
\draw[darrow] (rdb.west) -- node[above, sublabel]{grammar rules} (gv.east);

%% raga DB -> swara quantizer (drops south, goes left past gv, hooks into swq.east)
\draw[darrow] (rdb.south) -- ++(0,-0.5)
              -- ($ (swq.east) + (2.0cm+1.6cm, -0.5cm) $)
              -- ($ (swq.east) + (2.0cm+1.6cm, 0) $)
              -- node[above, sublabel]{scale ratios} (swq.east);

%% ── GROUP BOX : Core DSP ───────────────────────────────────────────────────
\begin{scope}[on background layer]
  \node[groupbox=blue,
        fit=(tsa)(swq)(gv),
        label={[font=\scriptsize\bfseries, text=blue!70]above:Core DSP Layer}] {};
\end{scope}
